...
Протокол RAdv это стандартный механизм динамической настройки конечных узлов. Он основан на протоколе ND и предназначен для передачи сообщений между маршрутизатор и его соседними узлами-абонентами (но никак не между маршрутизаторами). Эти сообщения, также как и другие ND-сообщения, не маршрутизируемы.

В рамках протокола каждый конечный узел отправляет Multicast-запрос Router Solicitation (RS). Адрес передачи — `0xff02::2` — описывает все доступные на данном сегменте связи (link-local) маршрутизаторы. То, является ли узел маршрутизатором, определяется по параметрам IP-forwarding-а. Со стороны маршрутизатора отправляется сообщение Router Advertisement (RA; важно не путать с протоколом, он RAdv). Маршрутизатор может отправлять RA по запросу RS на точный адрес конечного узла или широковещательно на `0xff02::1` с некоторым периодом. В случае, если сообщение RA приходит маршрутизатору, он просто его игнорирует. Вообще говоря, получая RA от маршрутизатора нельзя ничего сказать о его единственности.

В RA маршрутизатор передаёт информацию о себе и об известном ему окружении:
 + Параметры обрабатываемого им трафика;
 + Доступность маршрутизатора (router lifetime);
...
### Процесс автоматического назначения адресов с помощью RAdv
...
Ключевые элементы здесь:
 ...
 Теперь добавим на srv префикс для распространения. Это можно сделать разными способами: можно задать прямо в BIRD (тогда для него можно добавить параметры опции Prefix Information), а можно просто назначить на нужном интерфейсе адрес с соответствующим префиксом, RAdv будет распространять его автоматически:
...
Автоматически появился маршрут по умолчанию на LLA до srv (исключительно за счёт получения RA-сообщения от маршрутизатора), появился маршрут в указанную сеть, и в ней же был сгенерирован свой уникальный адрес.
...
При отключении BIRD отправляется завершающее RA-сообщение, отзывающее информацию о данном префиксе (`router lifetime 0s`)
...
Работает протокол с помощью всё тех же демонов маршрутизации. Для IPv6 особенно важна третья версия протокола, поскольку OSPFv3 работает только на link-local адресах, что упрощает его внутреннюю логику (не нужно лишний раз отслеживать, смаршрутизируемо ли сообщение от непрямого соседа или нет).
...
Для настройки адресов на абонентах будем использовать RAdv, а для передачи информации между маршрутизаторами — OSPF.
...
После запуска в течение некоторого времени маршрутизаторы обмениваются данными со всеми:
 + С абонентами относительно RAdv и выдачи им адресов
...
Если увеличивать сеть ещё больше, возникают новые проблемы. Даже при динамической маршрутизации объёмы покрываемых абонентов были бы настолько большими, что никаким образом не уместились бы в таблицы маршрутизации. Поэтому глобальная сеть разделяется на [_автономные системы_](https://ru.wikipedia.org/wiki/%D0%90%D0%B2%D1%82%D0%BE%D0%BD%D0%BE%D0%BC%D0%BD%D0%B0%D1%8F_%D1%81%D0%B8%D1%81%D1%82%D0%B5%D0%BC%D0%B0_%28%D0%98%D0%BD%D1%82%D0%B5%D1%80%D0%BD%D0%B5%D1%82%29). Это системы под единым юридическим, правовым администрированием, с единой политикой управления и маршрутизации. Внутри таких автономных систем развёртываются протоколы динамической маршрутизации, отлично решающие поставленные им задачи.
...
_Дополнительное условие_: никаких заранее заданных маршрутов в таблице маршрутизации ядра, маршруты приезжают по OSPF и RAdv
